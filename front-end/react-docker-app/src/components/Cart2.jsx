...
export const Cart = (props) => {
    ...
    useEffect(() => {
        ...
        NewCounter(exampleItem);
        /*console.log(counterArray);*/
    }, []);
    const NewCounter = (itemFetch) => {
        var initCounter =[]
        console.log(itemFetch, itemFetch.length);
       /* itemFetch.map(e => {
           //    setCounterArray[itemFetch,]
        })*/
        for (var i = 0; i < itemFetch.length; i++) {
            
            initCounter.push({
                name: exampleItem[i].name,
                value: exampleItem[i].price,
                counter: 1
            })
        }
        setCounterArray(initCounter);
        setLoad(true);
